...
knitr::opts_chunk$set(dev.args = list(png = list(type = "cairo")),
                      fig.path='out/figs/posterior/', dpi = 300)
```
...
if (redownload){
  ...
  data.exp <- rbind(
    read.csv(paste(src_folder, 'pm_exp_gbcs', file, sep = '/')) %>% mutate(experiment = 'gbcs'),
    read.csv(paste(src_folder, 'pm_exp_whas', file, sep = '/')) %>% mutate(experiment = 'whas'),
    read.csv(paste(src_folder, 'pm_exp_pbc', file, sep = '/')) %>% mutate(experiment = 'pbc'),
    read.csv(paste(src_folder, 'pm_exp_aids', file, sep = '/')) %>% mutate(experiment = 'actg')
  )
  
  
  data.wb <- rbind(
    read.csv(paste(src_folder, 'pm_wb_gbcs', file, sep = '/')) %>% mutate(experiment = 'gbcs', model = 'pm_wb'),
    read.csv(paste(src_folder, 'pm_wb_whas', file, sep = '/')) %>% mutate(experiment = 'whas', model = 'pm_wb'),
    read.csv(paste(src_folder, 'pm_wb_pbc', file, sep = '/')) %>% mutate(experiment = 'pbc', model = 'pm_wb'),
    read.csv(paste(src_folder, 'pm_wb_aids', file, sep = '/')) %>% mutate(experiment = 'actg', model = 'pm_wb')
  )
...
plot_density <- function(data, experiment_str, model_str, var_str){
  ...
  p1 <- ggdensity(data = data.filter %>% filter(var == var_str) ,
          ...
          facet.by = c('var', 'posterior_type_lbl' ),
...
plot_density_2 <- function(data, experiment_str, model_str, var_str, add_normal=FALSE) {
  data.filter <- data %>%
    filter(experiment == experiment_str & model == model_str) %>%
  mutate(
    partition_lbl = factor(partition, levels = paste("P", unique(partition), sep = "")
  ))
  
  p <- ggdensity(data = data.filter %>% filter(var == var_str) ,
            x = 'posterior_samples', y = 'density',
            add = 'median', title = var_str,
            color = 'posterior_type_lbl', size = 1, 
            fill = 'posterior_type_lbl', alpha = 0.1,
            facet.by = c('var', 'partition_lbl' ),
            palette = 'npg')
...
#   ))
#   
#   # dens <- density(data.filter$posterior_samples, 
#   #                 from = min(data.filter$posterior_samples)/4, 
#   #                 to = max(data.filter$posterior_samples)/4)
#   # 
#   # print(dens)
#   
#   # min_x <- dens$x[which.max(dens$y > 0.2)]
#   # # max_x <- dens$x[which.max(dens$y[:-20] > 0.05)]
#   # max_x <- max(data.filter$posterior_samples)
#   # print(min_x)
#   # print(max_x)
# 
#   norm_dens <- data.frame(samples = rnorm(100)) 
...
# }
# 
# p <- plot_density_2(data, 'whas', 'pm_exp', 'lambda_intercept', TRUE)
# 
# p
...
```{r}
# plot_density(data, 'whas', 'pm_exp', 'lambda_mitype_1')
# 
# plot_density_2(data, 'whas', 'pm_exp', 'lambda_mitype_1', TRUE)
# plot_density_2(data, 'whas', 'pm_exp', 'lambda_mitype_3', TRUE)
# plot_density_2(data, 'whas', 'pm_exp', 'lambda_chf', TRUE) + xlim(-2, 2)
```


```{r whas-posterior-type, fig.width=7, fig.asp=0.5}
