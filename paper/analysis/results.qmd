...
if (redownload) {
  ...
  data <- rbind(data.rsf, data.cox, data.dps, data.exp, data.wb)
...
data <- data.raw %>%
  ...
  mutate(experiment = ifelse(experiment == 'data/aids/', 'data/aids', experiment),
         ...
         model_lbl = factor(model, 
                            levels = c('cox', 'deepsurv', 'pmsurv_exponential', 'pmsurv_weibull_linear', 'rsf'),
                            labels = c("CoxPH", "DeepSurv", "PM Exp", "PM Wb", "RSF")))
